...
function angleBetweenClockHands(date) {
  const minuteAngle = date.getUTCMinutes() * 6;
  const hourAngle = 0.5 * (60 * (date.getUTCHours() % 12) + date.getUTCMinutes());
  const angleBeetwen = Math.abs(hourAngle - minuteAngle);

  const toRad = angleBeetwen <= 180
