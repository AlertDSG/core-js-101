...
function getPolynom(...args) {
  return args
    ? (arg) => args.reduce((acc, el, i) => acc + el * (arg ** (args.length - 1 - i)), 0)
    : null;
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const logParam = `${funcName}(${args.map((el) => JSON.stringify(el))})`;
    logFunc(`${logParam} starts`);
    const result = func(...args);
    logFunc(`${logParam} ends`);
    return result;
  };
}
